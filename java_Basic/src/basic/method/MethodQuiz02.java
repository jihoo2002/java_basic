...
static void push(String x) {
	...
	String[] arr = new String[foods.length+1]; //4
		for(int i= 0; i<foods.length; i++) { //0부터2번 인덱스
			arr[i] = foods[i];
			// arr.length로 한다면 반복문을 돌려 
...
static boolean inclode(String targetName) {
...
//전달된 음식을 배열에서 삭제하고 배열의 길이를 줄이면 된다.
	static void remove(String targetName) {
		int idx = indexOf(targetName); //인덱스 값옴
		if(idx == -1) {
...
//배열 내부를 한번 출력 후, '라면', '김치찌개' 추가한 뒤 배열 출력하기
        ...
        printFood();
        //라면의 존재 여부 출력
        System.out.println("라면의 존재여부: " +inclode("라면"));
        //양념치킨의 존재 여부 출력
        System.out.println("양념치킨의 존재여부: " +inclode("양념치킨"));        
        //2번 인덱스 데이터를 닭갈비로 수정
		modify(2, "닭갈비");
		printFood();
	}
