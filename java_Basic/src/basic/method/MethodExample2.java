...
public class MethodExample2 {
	...
	static String selectRandomFood() {
		double rn = Math.random(); //0.0~1.0미만
		if(rn > 0.66) {
			return "치킨";
		}else if(rn >0.33) {
			return "족발";
			...
			return "삽겹살";
		}//리턴하고자 하는 값의 타입과 함수 앞 데이터 타입이 같아야함
	}
...
//		} -> 향상 for문
		for(int i =0; i<nums.length; i++) {
			total += nums[i]; //배열 안 값의 총합
		}
		return total;
	}
	
	// 가변 인수 (가변 파라미터)를 사용한 매개변수 작성.
	//콤마로 나열되어 들어오는 여러개의 값을 배열로 묶어서 내부로 전달합니다.
	static int calcNumberTotal2(int... nums) { //점을 불이면 여러개를 전달할 수있는 크기로 변함
		int total =0;
		for( int n : nums) {
			total += n;
		}
	...
	public static void main(String[] args) {
		...
		//배열로 포장해서 보내도 된다.
		sum = calcNumberTotal2(1,2,3,4,5,6,7); //가변인수는 개수 상관없음
		System.out.println("누적합: " + sum);
