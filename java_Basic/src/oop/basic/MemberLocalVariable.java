...
public class MemberLocalVariable {
	...
	int a; //(멤버변수)필드 ,기본값으로 초기화되기에 값 지정 안해줘도 됨
	void printNumber(int c) { //c는 지역변수이자 매개변수
		int b =1;; //지역 변수
		System.out.println("멤버 변수 a:" + a);
		System.out.println("멤버 변수 b:" + b); //b의 값을 지정해주지 않아서!..
		System.out.println("멤버 변수 c:" + c);
	}
	
	public static void main(String[] args) {
		MemberLocalVariable m = new MemberLocalVariable(); //무조건 실체 생성하고
		m.printNumber(4);
