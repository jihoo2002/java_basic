...
public class DeleteFolder {
	...
	public static void main(String[] args) {
		 ...
		 * 폴더를 삭제할 수 없습니다.
		 * listFiles()를 통해 파일 내부 경로 또는 파일 명을 얻어와서 파일부터 삭제한 후에
		 * 폴더를 삭제할 수 있습니다.
		 * 지우고자 하는 경로가 여러개라면 위의 행동을 반복해주시면 됩니다. 
		 */
		if(file.exists()) {
	...
	//			System.out.println(Arrays.toString(files));
				...
				for(int i =0; i<files.length; i++) {
					if(files[i].delete()) {
						...
						System.out.println(files[i].getName() + "삭제 실패ㅜ");
					}
				}
		...
		if(file.exists()) {
			...
			System.out.println("삭제 실패 or 경로가 존재하지 않습니다.");
		}
		apple폴더 지움*/
